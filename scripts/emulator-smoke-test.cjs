/*
Emulator smoke test for coin awarding flows (CommonJS).
...
2) In another terminal, run this script:
   node scripts/emulator-smoke-test.cjs
...
It will:
 ...
 - create an offlineBooking (PENDING) then set it to SETTLED -> expect coins awarded
...
async function run() {
  ...
  const updatedUserSnap = (await db.collection('users').where('mobile','==',mobile).limit(1).get()).docs[0];
